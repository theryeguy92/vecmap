...
def print_matrix(
    ...
    print("=" * line_w)
    print(f"{'SOURCE SECTION':<{W_REG}}  {'BEST TARGET MATCH':<{W_PROC}}  "
          f"{'SIM':>{W_SIM}}  {'STATUS'}")
    print("-" * line_w)
    ...
    for i in range(M):
        ...
        print(f"{reg_label:<{W_REG}}  {proc_label:<{W_PROC}}  "
        ...
        rows.append({"reg_idx": i, "proc_idx": best_j, "sim": best_sim, "status": label})

    print("=" * line_w)
    coverage_pct = 100.0 * n_covered / M if M else 0.0
    print(f"{'SUMMARY':<{W_REG}}  "
