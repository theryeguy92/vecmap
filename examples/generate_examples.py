...
from graph.builder import load_embeddings  # noqa: E402
from output.matrix import print_matrix  # noqa: E402
from output.gap_report import print_gaps  # noqa: E402
from output.rankings import print_rankings  # noqa: E402
from output.knowledge_graph import export_dot  # noqa: E402
import regmap_cuda  # noqa: E402
...
def main() -> None:
    ...
    print(
        f"  Source corpus : {corpus_a.n} sections  "
        f"({', '.join(sorted({s.doc_id for s in corpus_a.sections}))})"
    )
    print(
        f"  Target corpus : {corpus_b.n} sections  "
        f"({', '.join(sorted({s.doc_id for s in corpus_b.sections}))})"
    )
    print(
        f"  Similarity    : {corpus_a.n} × {corpus_b.n} × 768 dims  "
